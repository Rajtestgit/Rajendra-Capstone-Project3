...
pipeline {
    environment {
        registry = "rajendocker/capstone-project3" //To push an image to Docker Hub, you must first name your local image using your Docker Hub username and the repository name that you created through Docker Hub on the web.
    ...
    stages {
        stage('GIT checkout') {
            steps {
               git branch: 'main', credentialsId: 'git-creds', url: 'https://github.com/Rajtestgit/Rajendra-Capstone-Project3.git' 
            }
        }
